...
export const ContrastChecker = (
  ...
  function getSelectedColorShades(
    selectedColor: 'all' | 'white' | COLOUR_NAMES
  ) {
  ...
  const selectedForegroundShades = getSelectedColorShades(
    selectedForgroundColor
  )
  const selectedBackgroundShades = getSelectedColorShades(
  ...
  return html`
    <table
      class="w-full"
      style="width: ${220 + numberOfForegroundColors * 50}px;"
    >
      <tr>
      ...
      <tr>
        ${Object.values(selectedForegroundShades).map(
          (color) =>
            html`<td
              class="size-50 border border-gray-600"
